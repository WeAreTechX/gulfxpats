...
async function testGulfJobsSystem() {
  ...
  try {
    ...
    const filesResponse = await makeRequest('/api/gulf-jobs?action=files');
    ...
    const scrapeResponse = await makeRequest('/api/gulf-jobs', 'POST', { forceScrape: true });
    ...
    console.log('\n🎉 Gulf Jobs System test completed!');
    console.log('\nNext steps:');
    console.log('1. Visit http://localhost:3000/admin/gulf-jobs to see the dashboard');
