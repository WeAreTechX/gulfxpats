...
async function triggerGulfJobs() {
  ...
  try {
    ...
    const scrapeResponse = await makeRequest('/api/jobs-scraper', 'POST', { 
      forceScrape: true 
    });
    ...
    console.log('\n🎉 Jobs System is ready!');
    console.log('\nNext steps:');
    console.log('1. Visit http://localhost:3000/jobs to see the jobs page');
    console.log('2. Visit http://localhost:3000/admin/jobs for the admin dashboard');
