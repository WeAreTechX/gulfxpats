...
import { ScrapedJob } from './gulf-job-scraper';
import { Job } from '@/types';

export interface GulfJobStorageConfig {
  dataDir: string;
...
export class GulfJobStorage {
  private config: GulfJobStorageConfig;
  private dataDir: string;

  constructor(config?: Partial<GulfJobStorageConfig>) {
    this.config = {
      dataDir: path.join(process.cwd(), 'data', 'gulf-jobs'),
      maxJobsPerFile: 1000,
      backupEnabled: true,
  ...
  async saveJobs(jobs: ScrapedJob[], filename?: string): Promise<string> {
    ...
    const defaultFilename = `gulf-jobs-${timestamp}.json`;
...
export const gulfJobStorage = new GulfJobStorage();
